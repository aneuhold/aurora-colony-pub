...
- **Type pairing**: one display face and one workhorse body face — pick a serif (or humanist sans with character) for display that can carry the heritage at large sizes, and a clean neutral for body. Set headlines large with tight leading; let body breathe. Land the choice in `--font-display` and `--font-body` so it's the only place to change later.
- **Color budget**: keep it tight — background, foreground, and one warm accent that reads like the room (think tungsten lamp on stained wood, not safety orange). Derive "muted" copy and hairline borders from `text-foreground/60` and `border-foreground/10` rather than minting new tokens. Add a second accent only when opacity-on-foreground gives the wrong hue (e.g. a true forest green for a "Family Friendly" or patio detail — propose, don't assume). The current values in `tokens.css` are placeholders; expect to retune them as part of any real design pass.
